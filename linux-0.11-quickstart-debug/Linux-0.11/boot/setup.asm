...
; posterity.
	...
	xor	bh,bh
	int	0x10		; save it in known place, con_init fetches;触发 BIOS 提供的显示服务中断处理程序;
...
; now we want to move to protected mode ...
	...
	cli			; no interrupts allowed ; 就一行 cli，表示关闭中断的意思。
...
do_move:
	...
	mov 	cx,0x8000
	rep
	movsw			; rep movsw 内存复制操作，把内存地址 0x10000 处开始往后一直到 0x90000 的内容，统统复制到内存的最开始的 0 位置。
	jmp	do_move
...
end_move:
	...
	mov	ds,ax
	lidt	[idt_48]		; load idt with 0,0 ; 中断描述符表（idt）
	lgdt	[gdt_48]		; load gdt with whatever appropriate; 全局描述符表（gdt）
